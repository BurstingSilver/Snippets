...
/*** Variables ***/
/**********************************************/
DECLARE @backupPath AS NVARCHAR(200) = N'Backup Path' --Example E:\Backup
DECLARE @sourceDBName AS NVARCHAR(50) = N'SourceDBName' --Example iMIS_Dev
DECLARE @destDBName AS NVARCHAR(50) = N'DestinationDBName' --Example iMIS_TEST
DECLARE @useDB VARCHAR(50)
Set @useDB =  'USE ' + @destDBName
DECLARE @fullPath AS NVARCHAR(254)
DECLARE @publichingCode AS varchar(5) = 'A'
DECLARE @sqlDataPath AS NVARCHAR(254) = N'E:\Program Files\Microsoft SQL Server\MSSQL12.MSSQLSERVER\MSSQL\DATA\' + @destDBName + N'_data.mdf'
DECLARE @sqlLogPath AS NVARCHAR(254) = N'E:\Program Files\Microsoft SQL Server\MSSQL12.MSSQLSERVER\MSSQL\DATA\' + @destDBName + N'_log.ldf'
DECLARE @imisInstallPath AS NVARCHAR(200) = N'C:\Program Files (x86)\ASI\iMISInstance\'
DECLARE @backupTestDB AS BIT = 0
DECLARE @executeSSISPackages AS BIT = 1
SET @fullPath = @backupPath + @sourceDBName + N'.bak'
DECLARE @StaffWebsiteRootUrl varchar(MAX) = 'http://YourDomain.Com/iMISInstance/'
DECLARE @StaffSecureWebsiteRootUrl varchar(MAX) = 'https://YourDomain.Com/iMISInstance/'
...
/*** Backup Database ***/
/**********************************************/
BACKUP DATABASE @sourceDBName TO  DISK = @fullPath WITH NOFORMAT, INIT,  NAME = N'Full Database Backup', SKIP, NOREWIND, NOUNLOAD,  STATS = 10
IF @backupTestDB = 1 BEGIN
       SET @fullPath = @backupPath + @destDBName + N'.bak'
       BACKUP DATABASE @destDBName TO  DISK = @fullPath WITH NOFORMAT, INIT,  NAME = N'Full Database Backup', SKIP, NOREWIND, NOUNLOAD,  STATS = 10
END
IF db_id(@destDBName) IS NOT NULL BEGIN
       -- DELETE Old DB
       EXEC msdb.dbo.sp_delete_database_backuphistory @database_name = @destDBName
       USE [master]
       EXEC ('ALTER DATABASE ' + @destDBName + ' SET  SINGLE_USER WITH ROLLBACK IMMEDIATE')
       EXEC ('DROP DATABASE ' + @destDBName)
END
/**********************************************/
/*** Restore Database ***/
/**********************************************/
SET @fullPath = @backupPath + @sourceDBName + N'.bak'
RESTORE DATABASE @destDBName FROM  DISK = @fullPath WITH  FILE = 1,  MOVE N'iMIS_Data' TO @sqlDataPath,  MOVE N'iMIS_Log' TO @sqlLogPath,  NOUNLOAD,  STATS = 5
/**********************************************/
/*** Switch to the other DB ***/
/**********************************************/
USE DestinationDBName --******************************Please change
--EXEC @useDB
